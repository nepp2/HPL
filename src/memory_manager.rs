// edited 2019, but just for formatting. Who knows what's going on in here.
...
impl<'a, 'b, 'c, 'd, 'e, 'f> BindingSectionMemoryManagerBuilder<'a, 'b, 'c, 'd, 'e, 'f> {
  ...
  pub fn create(mut self) -> BindingSectionMemoryManager<'a, 'b, 'c, 'd, 'e, 'f> {
    ...
    extern fn allocate_code_section_callback(
      opaque: *mut c_void,
    ...
    extern fn allocate_data_section_callback(
      opaque: *mut c_void,
      mm: LLVMMCJITMemoryManagerRef,
      size: uintptr_t,
      alignment: c_uint,
    ...
    extern fn finalize_memory_callback(
      opaque: *mut c_void,
      mm: LLVMMCJITMemoryManagerRef,
    ...
    extern fn invalidate_instruction_cache_callback(opaque: *mut c_void, mm: LLVMMCJITMemoryManagerRef) {
      let parent = ParentBSMM(mm);
      unsafe {
        let bsmm_functions = &mut *(opaque as *mut BSMMFunctions);
    ...
    extern fn get_symbol_address_callback(
      opaque: *mut c_void,
      mm: LLVMMCJITMemoryManagerRef,
    ...
    let allocate_code_section = unsafe {
      match self.bsmm_functions.allocate_code_section {
        ...
        None =>
          std::mem::transmute::<*mut c_void, wrappers::LLVM_BSMMAllocateCodeSectionCallback>(0 as *mut c_void),
      }
    };
    ...
    let allocate_data_section = unsafe {
      match self.bsmm_functions.allocate_data_section {
        ...
        None =>
          std::mem::transmute::<*mut c_void, wrappers::LLVM_BSMMAllocateDataSectionCallback>(0 as *mut c_void),
      }
    };
    ...
    let finalize_memory = unsafe {
      match self.bsmm_functions.finalize_memory {
        ...
        None =>
          std::mem::transmute::<*mut c_void, wrappers::LLVM_BSMMFinalizeMemoryCallback>(0 as *mut c_void),
      }
    };
    ...
    match self.bsmm_functions.invalidate_instruction_cache {
      Some(_) =>
        invalidate_instruction_cache_callback as wrappers::LLVM_BSMMInvalidateInstructionCacheCallback,
      None =>
        std::mem::transmute::<*mut c_void, wrappers::LLVM_BSMMInvalidateInstructionCacheCallback>(0 as *mut c_void),
    }
    };

    let get_symbol_address =  unsafe {
      match self.bsmm_functions.get_symbol_address {
        Some(_) => get_symbol_address_callback as wrappers::LLVM_BSMMGetSymbolAddressCallback,
        None =>
          std::mem::transmute::<*mut c_void, wrappers::LLVM_BSMMGetSymbolAddressCallback>(0 as *mut c_void),
      }
    };
    ...
    let destroy =  unsafe {
      match self.bsmm_functions.destroy {
        ...
        None =>
          std::mem::transmute::<*mut c_void, wrappers::LLVM_BSMMDestroyCallback>(0 as *mut c_void),
      }
    };
