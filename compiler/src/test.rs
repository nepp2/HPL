...
rusty_fork_test! {
  ...
  /// This sometimes fails in code generation, because the functions could be generated in either order due to iteration over secure hashmaps.
  /// If `a` is generated first, the compiler front-end generates the expected prototype for malloc. However, if `b` is generated first, a malloc
  /// prototype is generated by LLVM to support its instructionset (in this case it is allocating an array on the heap). This will change
  /// when arrays are stack allocated, but this shouldn't be possible anyway.
  /// LLVM wants malloc to take a u32, while I presumed it should take a u64. LLVM is probably right, so I should possibly just change my definition?
  #[test]
